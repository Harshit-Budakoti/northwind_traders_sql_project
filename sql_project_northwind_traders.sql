...
a."companyName" as name, sum((1-c."discount")*(c."unitPrice"*c."quantity")) as revenue , 
sum(b."freight") as freight ,
(sum((1-c."discount")*(c."unitPrice"*c."quantity"))-sum(b."freight")) as net_revenue
...
sum((1-a."discount")*(a."unitPrice"*a."quantity")) as revenue
from order_details as a join products as b 
on a."productID" = b."productID" 
group by a."productID",Item_name
...
rank() over (partition by b."productName" order by sum((1-a."discount")*(a."unitPrice"*a."quantity")) desc)
,d."companyName",sum((1-a."discount")*(a."unitPrice"*a."quantity")) as rev
from order_details as a join products as b 
on a."productID" = b."productID"
...
rank() over (partition by b."productName" order by sum((1-a."discount")*(a."unitPrice"*a."quantity")) desc)
,sum((1-a."discount")*(a."unitPrice"*a."quantity")) as rev
from order_details as a join products as b 
on a."productID" = b."productID"
join orders as c on a."orderID" = c."orderID"
join customers as d on c."customerID" = d."customerID"
...
sum((1-c."discount")*c."quantity"*c."unitPrice") as total_selling_value,
rank() over (partition by date_part('year',b."orderDate") order by 
			sum((1-c."discount")*c."quantity"*c."unitPrice") desc) as rnk
...
rank() over (partition by (date_part('year',b."orderDate"),date_part('quarter',b."orderDate"))
			 order by sum((1-c."discount")*c."quantity"*c."unitPrice") desc) as rnk
from employees as a JOIN orders as b on a."employeeID" = b."employeeID"
JOIN order_details as c on b."orderID" = c."orderID"
...
select d."employeeID" as id,d."employeeName" as name,b."city" as city,sum((1-c."discount")*c."quantity"*c."unitPrice") as city_rev
,first_value(b."city") over (partition by d."employeeID" order by sum((1-c."discount")*c."quantity"*c."unitPrice") desc ) as top_city
...
with emp_city as (select distinct d."employeeName" as name, b."city" as client_city,d."city" as employee_city,sum((1-c."discount")*c."quantity"*c."unitPrice") as city_rev,
				  b."country" as client_country
,rank() over (partition by d."employeeName" order by sum((1-c."discount")*c."quantity"*c."unitPrice") desc ) as rnk
from orders as a join customers as b on a."customerID"=b."customerID"
join order_details as c on a."orderID" = c."orderID" join employees as d on 
a."employeeID" = d."employeeID"
...
with qt as (select date_part('year',a."orderDate")
					   ...
					   sum((1-b."discount")*(b."unitPrice"*b."quantity"))as rev
...
with qt2 as(select date_part('year',c."orderDate")
			           ...
			           ,sum((1-a."discount")*(a."unitPrice"*a."quantity")) as rev,
					   rank() over(partition by (date_part('year',c."orderDate"),date_part('quarter',c."orderDate"))order by sum((1-a."discount")*(a."unitPrice"*a."quantity")) desc ) as rnk,
	...
	qt3 as (select date_part('year',c."orderDate")
			           ...
			           ,sum((1-a."discount")*(a."unitPrice"*a."quantity")) as t_rev
...
with qt2 as(select date_part('year',c."orderDate")
					   as Year,date_part('quarter',c."orderDate") as quarter,b."productName" as product
			           ,sum((1-a."discount")*(a."unitPrice"*a."quantity")) as rev,
					   cume_dist() over(partition by (date_part('year',c."orderDate"),date_part('quarter',c."orderDate"))order by sum((1-a."discount")*(a."unitPrice"*a."quantity")) asc ) as cum_dist,
			avg(c."requiredDate"-c."orderDate") as exp_lead_time ,avg(c."shippedDate"-c."orderDate") as act_lead_time,avg(freight) as avg_shipper_fees
					   from order_details as a join products as b on a."productID" = b."productID"
			           join orders as c on a."orderID" = c."orderID"
					  group by Year,quarter,b."productName"
...
with q_c_shipment as (select concat(cast(date_part('year',a."orderDate") as text),' Quarter-',
			  ...
			  sum((1-c."discount")*(c."unitPrice"*c."quantity")) as total_freight
			  ,rank() over (partition by (concat(cast(date_part('year',a."orderDate") as text),' Quarter-',
			  cast(date_part('quarter',a."orderDate") as text)),b.city) order by sum((1-c."discount")*(c."unitPrice"*c."quantity")) desc) as rank
...
with q_cntry_shipment as (select concat(cast(date_part('year',a."orderDate") as text),' Quarter-',
			  ...
			  ,rank() over (partition by (concat(cast(date_part('year',a."orderDate") as text),' Quarter-',
			  cast(date_part('quarter',a."orderDate") as text)),b.country) order by sum((1-c."discount")*(c."unitPrice"*c."quantity")) desc) as rank
